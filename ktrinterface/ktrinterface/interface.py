...
"""
#TODO: Allow for extra args/kwargs to be passed to objective, constraint, etc.
#TODO: See how AMPL and Matlab deal with constants, eg. KTR_INFBOUND

from __future__ import print_function, division
from ktrinterface.knitro import *
...
class Interface(object):
    ...
    def _options_default(self):
        """ Default solver options for Interface. Set expectation of analytical
        gradient/Jacobian and Hessian if provided.
        """
    ...
    def _ktr_defs_default(self):
        ...
        """
        out = {}
        # Control for scalar guess for one-dimesional problem
        if (not isinstance(self.guess, np.ndarray) and
                not isinstance(self.guess, list)):
            self.guess = [self.guess]
        out['n_params'] = len(self.guess)
        out['obj_goal'] = KTR_OBJGOAL_MINIMIZE
        out['obj_type'] = KTR_OBJTYPE_GENERAL
        out['bnds_lo'] = np.array([-KTR_INFBOUND]).repeat(out['n_params'])
        out['bnds_up'] = np.array([KTR_INFBOUND]).repeat(out['n_params'])
        if 'constr' in self.kwargs:
            out['m_constr'] = len(self.kwargs['constr'](self.guess))
            out['c_type'] = np.array([KTR_CONTYPE_GENERAL]).\
                            repeat(out['m_constr'])
            out['c_bnds_lo'] = np.array([-KTR_INFBOUND]).\
                               repeat(out['m_constr'])
            out['c_bnds_up'] = np.array([KTR_INFBOUND]).\
                               repeat(out['m_constr'])
            ...
            # Turn sparsity structure into KNITRO-readable format
    ...
    def _param_set(self):
        ...
        for name, option in self.options.iteritems():
            if isinstance(option, str):
                if KTR_set_char_param_by_name(self.ktr_current, name, option):
                    raise RuntimeError("Error setting parameter '" +
                                       name + "'")

            elif isinstance(option, int):
                if KTR_set_int_param_by_name(self.ktr_current, name, option):
                    raise RuntimeError("Error setting parameter '" +
                                       name + "'")

            elif isinstance(option, float):
                if KTR_set_double_param_by_name(self.ktr_current, name,
                                                option):
                    raise RuntimeError("Error setting parameter '" +
                                       name + "'")
    ...
    def _eval_ga(self, coef_out, obj_grad, jac):
        ...
        #TODO: Change this to take the full matrix rather than strung-out vector
    ...
    def _callback_eval_fc(self, eval_request_code, n_params, m_constr, nnz_j,
        ...
        if eval_request_code == KTR_RC_EVALFC:
    ...
    def _callback_eval_ga(self, eval_request_code, n_params, m_constr, nnz_j,
        ...
        if eval_request_code == KTR_RC_EVALGA:
    ...
    def _callback_eval_h(self, eval_request_code, n_params, m_constr, nnz_j,
        ...
        if eval_request_code == KTR_RC_EVALH:
            self._eval_h(coef_out, lambda_, 1.0, hessian)
            return 0

        elif eval_request_code == KTR_RC_EVALH_NO_F:
    ...
    def _callback_reg(self):
        ...
        if KTR_set_func_callback(self.ktr_current, self._callback_eval_fc):
            raise RuntimeError("Error registering function callback.")

        if 'grad' in self.kwargs:
            if KTR_set_grad_callback(self.ktr_current, self._callback_eval_ga):
                raise RuntimeError("Error registering gradient callback.")

        if 'hess' in self.kwargs:
            if KTR_set_hess_callback(self.ktr_current, self._callback_eval_h):
    ...
    def _init_problem(self):
        ...
        return KTR_init_problem(self.ktr_current,
                                    self.ktr_defs['n_params'],
    ...
    def solve(self, verbose=True):
        ...
        self._out_init()
        n_status = KTR_solve(self.ktr_current, self.out['coef'],
                                 self.out['lambda_'], 0,
        ...
        if n_status != 0:
            ...
            if verbose:
                print("KNITRO successful, feasibility violation    "
                      "= %e" % KTR_get_abs_feas_error(self.ktr_current))
                print("                   KKT optimality violation "
                      "= %e" % KTR_get_abs_opt_error(self.ktr_current))
        KTR_free(self.ktr_current)
        return self.out
